...
if(!empty($_POST)) {
	...
	if(empty($_SESSION['error'])) {
		...
		$stmt = $database->prepare("UPDATE `users` SET `name` = ?, `email` = ? WHERE `user_id` = {$_GET['user_id']}");
...
<h4><?php echo $language['headers']['change_details']; ?></h4>
...
<form action="" method="post" role="form">
    ...
    <h4><?php echo $language['headers']['change_password']; ?></h4>
